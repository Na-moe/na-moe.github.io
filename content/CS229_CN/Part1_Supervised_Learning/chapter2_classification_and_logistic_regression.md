...
| [[chapter1_linear_regression\|上一章]] | [[CS229_CN/index#目录\|目录]] | [[chapter3_generalized_linear_model\|下一章]] |
| :---------------------------------: | :--------------------: | :----------------------------------------: |
...
**备注 2.1.1.** 同一个损失函数的另一种表示方式也很有用，特别是在第 [[chapter7_deep_learning#7.1 使用非线性模型的监督学习|7.1]] 节研究非线性模型时。 ^rmk2-1-1
...
这与方程 [[chapter2_classification_and_logistic_regression#^eq2-2|(2.2)]] 的推导是一致的。在第 [[chapter7_deep_learning#7.1 使用非线性模型的监督学习|7.1]] 节中，会看到这种观点可以扩展到非线性模型。
...
在最左边的图中，可以看到函数 $f$ 与直线 $y=0$ 相交。尝试找到一个 $\theta$ 使得 $f(\theta)=0$; 这一点的 $\theta$ 值大约是 $1.3$. 假设初始化的算法的 $\theta$ 值为 $4.5$. 然后牛顿法拟合一条在 $\theta=4.5$ 处与 $f$ 相切的直线，并求解该直线等于 $0$ 的点（中间图）。这给出了 $\theta$ 的下一个猜测值，大约是 $2.8$. 最右边的图显示了再进行一次迭代的结果，更新后的 $\theta$ 大约是 $1.8$. 再经过几次迭代后，将迅速接近 $\theta = 1.3$. 
