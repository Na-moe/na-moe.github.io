...
通过在模型未训练过的例子集合上进行测试/验证，我们获得了每个假设 $h_i$ 的真实泛化/测试误差的更好估计。因此，这种方法本质上是选择具有最小估计泛化/测试误差的模型。验证集的大小取决于可用例子的总数。通常，留出交叉验证中使用的验证集数据量占总数据量的 1/4 到 1/3，其中 30\% 是一个典型的选择。然而，当总数据集非常大时，只要验证例子的绝对数量足够多，验证集可以占总例子的一小部分。例如，ImageNet 数据集有大约 1M 训练图像，验证集有时设置为 50K 图像，仅占总例子的约 5\%.
