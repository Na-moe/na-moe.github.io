...
Some studies \cite{zhang2022batch,ali2020batch,lu2018crowdvision} investigate dynamically adjusting batch size under dynamic workloads based on different levels of task arrival information. In the absence of any prior information about task arrivals, Zhang et al. in \cite{zhang2022batch} utilize a reinforcement learning approach to predict future task arrival patterns and adjust batch sizes accordingly. When task arrival patterns can be determined, such as Poisson processes and Markov processes, Ali et al. in \cite{ali2020batch} model the probability distribution of latency concerning batch size and search for optimal batch sizes to minimize latency or cost. When accurate arrival times are known, Lu et al. in \cite{lu2018crowdvision} develop a Split-Shift method for batch allocation. However, they greedily split batches to minimize waiting time, which achieves a sub-optimal trade-off between computational efficiency and waiting time and damages the overall performance.

### C. Batching with Task Steering

Some studies \cite{cui20202,zhang2023bcedge} focus on integrating task steering with static batching. They treat batching as a limiting condition within their problem formulations. For example, Cui et al. in \cite{cui20202} propose to steer tasks to different devices with preset fixed batch sizes to balance the remaining memory size. Similarly, Zhang et al. in \cite{zhang2023bcedge} employ reinforcement learning to decide on task steering to instances with static batch configurations to maximize throughput.

Recent studies (e.g., \cite{zhang2019edgebatch}) have explored the coordination of dynamic batching and task steering across multiple edge servers. Zhang et al. in \cite{zhang2019edgebatch} employ a stochastic approach for optimal task batching based on a probability model, using an online regret minimization method, and address task steering through a warehouse model. However, they do not jointly optimize batching and steering. Instead, they define a utilization threshold for each server based on static parameters that estimate batch efficiency, directing tasks to servers only when this threshold is not exceeded. This approach limits the adaptability and effectiveness of task distribution in dynamic edge environments.
...
We illustrate the measurement of batching efficiency across different hardware platforms \cite{nvidia2018v100,nvidia2020jax} and popular neural networks \cite{he2016resnet,simonyan2014vgg,szegedy2015googlenet} in Fig. \ref{fig:batch eff}. Note that the maximum permissible batch sizes on different platforms are constrained by memory limitations and the neural networks adopted. For example, in our measurement, the maximum batch size on the NVIDIA Tesla V100 is about 128, whereas, on the NVIDIA Jetson AGX Xavier, it is limited to 32 or 16.  
